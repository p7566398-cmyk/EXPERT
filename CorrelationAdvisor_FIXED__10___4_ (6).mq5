...
input group "=== ОБЩИЕ ПАРАМЕТРЫ (ALL SET) ==="
// Используются когда кнопка ALL SET активна (фиолетовая)
...
input double InpAllExtSLB   = 60.0;   // ALL: SL блока %

input group "=== АВТОЛОТ (A-LOT) ==="
// Используются когда кнопка A-LOT активна (Peru)
input double InpALotDivisor = 1000.0; // A-LOT: делитель депозита (депозит/N = mainLot)
input double InpALotSafeDiv = 2.5;    // A-LOT: кратность (mainLot/N = safeLot)
...
input double InpSet1MainLot = 0.10;          // Сет 1: лот основной
input double InpSet1CorrLot = 0.10;          // Сет 1: лот корреляционный
input double InpSet1SafeLot = 0.03;          // Сет 1: лот SAFE
input double InpSet1E1Lot   = 0.03;          // Сет 1: лот EXTRA 1
input double InpSet1E2Lot   = 0.03;          // Сет 1: лот EXTRA 2
input double InpSet1E3Lot   = 0.04;          // Сет 1: лот EXTRA 3
input int    InpSet1TP      = 100;           // Сет 1: TP (пункты)
input int    InpSet1SL      = 120;           // Сет 1: SL (пункты)
input double InpSet1ClosePct= 0.5;           // Сет 1: закрытие % от TP
input double InpSet1ExtDev  = 20.0;          // Сет 1: ШАГ EXTRA %
input double InpSet1ExtCls  = 0.8;           // Сет 1: закрытие EXTRA %
input double InpSet1ExtSLB  = 60.0;          // Сет 1: SL блока %
...
input double InpSet2MainLot = 0.10;          // Сет 2: лот основной
input double InpSet2CorrLot = 0.10;          // Сет 2: лот корреляционный
input double InpSet2SafeLot = 0.03;          // Сет 2: лот SAFE
input double InpSet2E1Lot   = 0.03;          // Сет 2: лот EXTRA 1
input double InpSet2E2Lot   = 0.03;          // Сет 2: лот EXTRA 2
input double InpSet2E3Lot   = 0.04;          // Сет 2: лот EXTRA 3
input int    InpSet2TP      = 100;           // Сет 2: TP (пункты)
input int    InpSet2SL      = 120;           // Сет 2: SL (пункты)
input double InpSet2ClosePct= 0.5;           // Сет 2: закрытие % от TP
input double InpSet2ExtDev  = 20.0;          // Сет 2: ШАГ EXTRA %
input double InpSet2ExtCls  = 0.8;           // Сет 2: закрытие EXTRA %
input double InpSet2ExtSLB  = 60.0;          // Сет 2: SL блока %
...
input double InpSet3MainLot = 0.10;          // Сет 3: лот основной
input double InpSet3CorrLot = 0.10;          // Сет 3: лот корреляционный
input double InpSet3SafeLot = 0.03;          // Сет 3: лот SAFE
input double InpSet3E1Lot   = 0.03;          // Сет 3: лот EXTRA 1
input double InpSet3E2Lot   = 0.03;          // Сет 3: лот EXTRA 2
input double InpSet3E3Lot   = 0.04;          // Сет 3: лот EXTRA 3
input int    InpSet3TP      = 100;           // Сет 3: TP (пункты)
input int    InpSet3SL      = 120;           // Сет 3: SL (пункты)
input double InpSet3ClosePct= 0.5;           // Сет 3: закрытие % от TP
input double InpSet3ExtDev  = 20.0;          // Сет 3: ШАГ EXTRA %
input double InpSet3ExtCls  = 0.8;           // Сет 3: закрытие EXTRA %
input double InpSet3ExtSLB  = 60.0;          // Сет 3: SL блока %
...
input double InpSet4MainLot = 0.10;          // Сет 4: лот основной
input double InpSet4CorrLot = 0.10;          // Сет 4: лот корреляционный
input double InpSet4SafeLot = 0.03;          // Сет 4: лот SAFE
input double InpSet4E1Lot   = 0.03;          // Сет 4: лот EXTRA 1
input double InpSet4E2Lot   = 0.03;          // Сет 4: лот EXTRA 2
input double InpSet4E3Lot   = 0.04;          // Сет 4: лот EXTRA 3
input int    InpSet4TP      = 100;           // Сет 4: TP (пункты)
input int    InpSet4SL      = 120;           // Сет 4: SL (пункты)
input double InpSet4ClosePct= 0.5;           // Сет 4: закрытие % от TP
input double InpSet4ExtDev  = 20.0;          // Сет 4: ШАГ EXTRA %
input double InpSet4ExtCls  = 0.8;           // Сет 4: закрытие EXTRA %
input double InpSet4ExtSLB  = 60.0;          // Сет 4: SL блока %
...
struct SBlock
{
   ...
   double  extraLevel5;   // L5: разблокирует закрытие EXTRA3
   double  extraLevel6;   // L6: стоп-лосс блока (= L1 - 5*step)
   bool    visitedLevel1;
   bool    visitedLevel2;
   bool    visitedLevel3;
   bool    visitedLevel4;
   ...
   SBlock()
   {
      ...
      extraLevel1=0; extraLevel2=0; extraLevel3=0; extraLevel4=0; extraLevel5=0; extraLevel6=0;
      visitedLevel1=false; visitedLevel2=false; visitedLevel3=false;
      visitedLevel4=false; visitedLevel5=false; visitedLevel6=false;
...
bool g_allSetActive = false; // ALL SET активна
bool g_aLotActive   = false; // A-LOT активна
...
void FillBlockFromSet(SBlock &blk, int setNum)
{
   ...
   {
      case 1:
         blk.mainPair=InpSet1MainPair; blk.corrPair=InpSet1CorrPair;
         blk.direction=InpSet1Direction; blk.correlation=InpSet1Corr;
         blk.mainLot=InpSet1MainLot; blk.corrLot=InpSet1CorrLot; blk.safeLot=InpSet1SafeLot;
         blk.extra1Lot=InpSet1E1Lot; blk.extra2Lot=InpSet1E2Lot; blk.extra3Lot=InpSet1E3Lot;
         blk.tp=InpSet1TP; blk.sl=InpSet1SL;
         blk.closePercent=InpSet1ClosePct;
         blk.extraDeviation=InpSet1ExtDev; blk.extraClosePercent=InpSet1ExtCls;
         blk.extraThreshold=InpSet1ExtSLB;
         break;
      case 2:
         blk.mainPair=InpSet2MainPair; blk.corrPair=InpSet2CorrPair;
         blk.direction=InpSet2Direction; blk.correlation=InpSet2Corr;
         blk.mainLot=InpSet2MainLot; blk.corrLot=InpSet2CorrLot; blk.safeLot=InpSet2SafeLot;
         blk.extra1Lot=InpSet2E1Lot; blk.extra2Lot=InpSet2E2Lot; blk.extra3Lot=InpSet2E3Lot;
         blk.tp=InpSet2TP; blk.sl=InpSet2SL;
         blk.closePercent=InpSet2ClosePct;
         blk.extraDeviation=InpSet2ExtDev; blk.extraClosePercent=InpSet2ExtCls;
         blk.extraThreshold=InpSet2ExtSLB;
         break;
      case 3:
         blk.mainPair=InpSet3MainPair; blk.corrPair=InpSet3CorrPair;
         blk.direction=InpSet3Direction; blk.correlation=InpSet3Corr;
         blk.mainLot=InpSet3MainLot; blk.corrLot=InpSet3CorrLot; blk.safeLot=InpSet3SafeLot;
         blk.extra1Lot=InpSet3E1Lot; blk.extra2Lot=InpSet3E2Lot; blk.extra3Lot=InpSet3E3Lot;
         blk.tp=InpSet3TP; blk.sl=InpSet3SL;
         blk.closePercent=InpSet3ClosePct;
         blk.extraDeviation=InpSet3ExtDev; blk.extraClosePercent=InpSet3ExtCls;
         blk.extraThreshold=InpSet3ExtSLB;
         break;
      case 4:
         blk.mainPair=InpSet4MainPair; blk.corrPair=InpSet4CorrPair;
         blk.direction=InpSet4Direction; blk.correlation=InpSet4Corr;
         blk.mainLot=InpSet4MainLot; blk.corrLot=InpSet4CorrLot; blk.safeLot=InpSet4SafeLot;
         blk.extra1Lot=InpSet4E1Lot; blk.extra2Lot=InpSet4E2Lot; blk.extra3Lot=InpSet4E3Lot;
         blk.tp=InpSet4TP; blk.sl=InpSet4SL;
         blk.closePercent=InpSet4ClosePct;
         blk.extraDeviation=InpSet4ExtDev; blk.extraClosePercent=InpSet4ExtCls;
         blk.extraThreshold=InpSet4ExtSLB;
         break;
   }
   // ALL SET: перезаписываем числа из общего блока
   if(g_allSetActive) ApplyAllSetNums(blk);
   // A-LOT: перезаписываем лоты по депозиту
...
void StartPhase2(SBlock &blk, int blockNum)
{
   ...
   // Шаг одинаковый для всех уровней L1–L6
   double step        = blk.takenProfit * blk.extraDeviation / 100.0;
   blk.extraLevel1    = -blk.takenProfit;
   blk.extraLevel2    = blk.extraLevel1 - step;       // EXTRA1 открывается
   blk.extraLevel3    = blk.extraLevel1 - step * 2.0; // EXTRA2 открывается
   blk.extraLevel4    = blk.extraLevel1 - step * 3.0; // EXTRA3 открывается
   blk.extraLevel5    = blk.extraLevel1 - step * 4.0; // разблокирует закрытие EXTRA3
   blk.extraLevel6    = blk.extraLevel1 - blk.takenProfit * blk.extraThreshold / 100.0; // L6 = SL блока
   blk.visitedLevel1  = true;  // первое открытие EXTRA1 разрешено
   blk.visitedLevel2  = true;  // первое открытие EXTRA2 разрешено
   blk.visitedLevel3  = true;  // первое открытие EXTRA3 разрешено
   blk.visitedLevel4  = false;
   blk.visitedLevel5  = false;
   blk.visitedLevel6  = false;
   ...
   Print("Блок ", blockNum, " EXTRA уровни:",
         ...
         " L6(SL)=", DoubleToString(blk.extraLevel6,2),
...
void CheckPhase2(SBlock &blk, int blockNum)
{
   ...
   // СТОП-ЛОСС (L6): срабатывает когда суммарный баланс блока достигает L6
   // L6 = L1 - 5*step = -TP - 5*(TP*deviation/100)
   // totalBal включает: Main2 + Corr2 + SAFE + EXTRA1/2/3
   if(blk.profitTaken && blk.takenProfit > 0 && blk.extraLevel6 < 0)
   {
      if(totalBal <= blk.extraLevel6)
      {
         Print("Блок ", blockNum, ": SL по L6. totalBal=", DoubleToString(totalBal,2),
               "$ <= L6=", DoubleToString(blk.extraLevel6,2),
               "$ (TP=", DoubleToString(blk.takenProfit,2),
               "$, шаг=", DoubleToString(blk.takenProfit*blk.extraDeviation/100.0,2), "$)");
...
void SaveBlockState(SBlock &blk, string pfx)
{
   ...
   GlobalVariableSet(GV(pfx+"vis6"),       blk.visitedLevel6 ? 1.0 : 0.0);
   // Уровни EXTRA
   ...
   GlobalVariableSet(GV(pfx+"lvl6"),  blk.extraLevel6);
...
bool RestoreBlockState(SBlock &blk, string pfx)
{
   ...
   blk.visitedLevel6 = GlobalVariableGet(GV(pfx+"vis6")) > 0.5;
   ...
   blk.extraLevel6 = GlobalVariableGet(GV(pfx+"lvl6"));
...
void CreatePanel()
{
   ...
   // ALL SET — справа от BLOCK1
   int allSetX = COL1_X + btnW0 + GAP;
   color C_ALLSET_OFF = C'60,60,60';  // серый когда неактивна
   color C_ALLSET_ON  = C'147,112,219'; // MediumPurple
   PanelBtn(g_prefix+"BTN_ALLSET", "ALL SET", allSetX, yB, 76, RHB, C_ALLSET_OFF, C_TXT_W);
   PanelBtn(g_prefix+"BTN_B2TOGGLE", "BLOCK 2", COL2_X, yB, btnW0, RHB, C_BTN_RED, C_TXT_W);
   // A-LOT — справа от BLOCK2
   int aLotX = COL2_X + btnW0 + GAP;
   color C_ALOT_OFF = C'60,60,60';  // серый когда неактивна
   color C_ALOT_ON  = C'205,133,63'; // Peru
   PanelBtn(g_prefix+"BTN_ALOT", "A-LOT", aLotX, yB, 76, RHB, C_ALOT_OFF, C_TXT_W);
...
void UpdatePanel()
{
   ...
   ObjectSetInteger(0, g_prefix+"BTN_ALLSET", OBJPROP_BGCOLOR,
      g_allSetActive ? (color)C'147,112,219' : (color)C'60,60,60');
   ObjectSetInteger(0, g_prefix+"BTN_ALOT",   OBJPROP_BGCOLOR,
      g_aLotActive   ? (color)C'205,133,63'  : (color)C'60,60,60');
...
void ShowExtraLevels(SBlock &blk, int blockNum)
{
   ...
   lvls[5].lbl="L6 [SL]";   lvls[5].val=blk.extraLevel6; lvls[5].isTech=true;
...
//============================================================
//  A-LOT: автоматический расчёт лотов по депозиту
//============================================================
double RoundLot(double lot) { return MathFloor(lot * 100.0 + 0.5) / 100.0; }
...
void CalcAutoLots(SBlock &blk)
{
   ...
   double e12     = RoundLot(MathFloor((mainLot - safeLot) / 3.0 * 100.0) / 100.0);
   double e3      = RoundLot(mainLot - safeLot - e12 - e12);
   blk.mainLot = mainLot; blk.corrLot = mainLot; blk.safeLot = safeLot;
   blk.extra1Lot = e12; blk.extra2Lot = e12; blk.extra3Lot = e3;
   Print("A-LOT: bal=", DoubleToString(balance,2), " main=", DoubleToString(mainLot,2),
         " safe=", DoubleToString(safeLot,2), " e1/e2=", DoubleToString(e12,2),
         " e3=", DoubleToString(e3,2),
         " check=", DoubleToString(mainLot-safeLot-e12-e12-e3, 3));
}

//============================================================
//  ALL SET: перезаписать числовые параметры блока из общих
//============================================================
void ApplyAllSetNums(SBlock &blk)
{
   blk.tp              = InpAllTP;    blk.sl          = InpAllSL;
   blk.extraDeviation  = InpAllExtDev; blk.extraClosePercent = InpAllExtCls;
   blk.extraThreshold  = InpAllExtSLB;
   blk.mainLot = InpAllMainLot; blk.corrLot = InpAllMainLot;
   blk.safeLot = InpAllSafeLot;
   blk.extra1Lot = InpAllE1Lot; blk.extra2Lot = InpAllE2Lot; blk.extra3Lot = InpAllE3Lot;
}
...
void ApplySetToBlock(SBlock &blk, int setNum)
{
   ...
   // Применяем ВСЕ параметры сета включая пары, направление, корреляцию
   switch(setNum)
   {
      case 1:
         blk.mainPair=InpSet1MainPair; blk.corrPair=InpSet1CorrPair;
...
void OnButtonClick(string name)
{
   ...
   //--- BLOCK кнопки — только ВКЛЮЧАЮТ блок в стендбай (нельзя нажать если уже включён)
      ...
      Print("ALL SET: ", g_allSetActive ? "ON — общие параметры" : "OFF — параметры из сетов");
   }
   else if(btn=="BTN_ALOT") {
      g_aLotActive = !g_aLotActive;
      if(g_aLotActive)
         Print("A-LOT ON: баланс=", DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE),2),
               "$ div=", InpALotDivisor, " safeDiv=", InpALotSafeDiv);
      else
         Print("A-LOT OFF");
   }
